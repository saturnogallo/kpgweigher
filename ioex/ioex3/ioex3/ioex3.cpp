...
int flash_cmd_handler(){
	...
	if(cmdid == "p1_chkpwd"){	//check password on page one :  p1_chkpwd:user:pwd	return p1_chkpwd_pass or p1_chkpwd_fail
		...
		*/
		...
		answer_flash(_T("updatepacker:0:")+packercfg);
		...
		setnodexml(0xff,toxml);		
		Sleep(toxml.GetLength()*10);
		answer_flash(_T("updatepacker:1:")+packercfg);
		...
		setnodexml(0xff,toxml);		
		Sleep(toxml.GetLength()*10);
		answer_flash(_T("updatepacker:2:")+packercfg);
		...
		setnodexml(0xff,toxml);		
		Sleep(toxml.GetLength()*10);
		answer_flash(_T("updatepacker:3:")+packercfg);
		...
		if(cmdid == "startrun")
		{
			...
			if(0 == localtime_s(&t_now,&t))
			{
				...
				ymd.Format("%i%02i%02i_%02i%02i%02i.txt",1900+t_now.tm_year,t_now.tm_mon+1,t_now.tm_mday,t_now.tm_hour,t_now.tm_min,t_now.tm_sec);
				s_starttime.Format("%i-%02i-%02i %02i:%02i:%02i",1900+t_now.tm_year,t_now.tm_mon+1,t_now.tm_mday,t_now.tm_hour,t_now.tm_min,t_now.tm_sec);
		...
		if(cmdid == "stoprun")
		{
			...
			if(0 == localtime_s(&t_now,&t))
			{
				...
				s_endtime.Format(_T("%i-%02i-%02i %02i:%02i:%02i"),1900+t_now.tm_year,t_now.tm_mon+1,t_now.tm_mday,t_now.tm_hour,t_now.tm_min,t_now.tm_sec);
				CString srecord;
				if (g_wcount > 0)
				{
					srecord.Format(_T("<node type='%s' start='%s' end='%s' number='%i' weight='%.2d'/>"),s_type,s_starttime,s_endtime,g_wcount,g_wavg);
					XmlAppend(srecord);
				}
		...
		*/
		...
		if(cmdid == "initpacker") //format initpacker:grp:config
		{
			...
			answer_flash(_T("updatepacker:")+sgrp+_T(":")+trigtype);
			toxml.Format(_T("%i"),atoi(trigtype));
		...
		if (cmdid == "his_delete") //format his_delete:start day::end day
		{
			CString sstart = cmdparm.Left(cmdparm.Find("::"));
			cmdparm.Delete(0,sstart.GetLength() + 2);

			XmlDelete(sstart,cmdparm);
		}
		return 1;
}
/*********************************************************************************************************************/
