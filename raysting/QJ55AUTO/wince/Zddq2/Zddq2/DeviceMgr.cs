...
02/0B(ON), J0 -- 1 ohm
03/0A -NA
04/0D(ON), J10 -- BACKUP 3
05/0C(ON), J11 -- BACKUP 4  --5A
06/0F(ON), J8 -- BACKUP 1 -- overlap - GAOZU,
07/0E(ON), J9 -- BACKUP 2
10/19(ON), 	J7 --- 2048
11/18(ON) ---- 8
12/1B(ON), J5 ---512
13/1A(ON) ---- 32
...
*/
    internal class DeviceMgr
    {
        ...
        {
            /*
            string reply = String.Format("D{0:D2}Measurements:R{1},S{2},#{3},&{4}\r", Program.lst_rxinfo[0].dRxInput.ToString("F6"),
                Program.lst_rsinfo[0].dTValue.ToString("F6"),
                index.ToString(),
                value.ToString("E11"));
            cmdport.WriteLine(reply);
             */
        ...
        static public void RelayState(string swi, string ktt, string curr, string sqr, string std, string constv)
        {
            ...
            if (sqr.StartsWith("X"))
            {
                if ((curr == "CURR_P3") || (curr == "CURR_1") || (curr == "CURR_5"))
                {
                    ...
                    if (sqr == "X2")
                        DelayWrite(new byte[] { 0x2A }, 0, 1);
                    if (sqr == "X1")
                        DelayWrite(new byte[] { 0x23 }, 0, 1);
                }
                else
                {
