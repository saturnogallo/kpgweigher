...
TESTERCONF g_TesterConf;		//tester configure
extern CString g_Path;				//app path
extern CTesterData g_Data;			//tester result

int CTesterProgram::State=BEIDLE;
...
bool CTesterProgram::InstallScheme(CString schemename)
{
	...
	while(csf.ReadString(line))
	{
		if(line.GetAt(0) == '<')//function declaration
		{
			...
			m_fun.SetAt(0,m_fun.GetAt(0)+line.Mid(1,pos-1)+'@'+str+',');
			while(csf.ReadString(line))
			{
				if(line.GetLength() < 1)
					continue;
...
void CTesterProgram::SetFunctionPointer(CString funname)
{
	if(State != BEIDLE)
		return;
	
	CString funlist=m_fun.GetAt(0);
	int pos = funlist.Find(funname);
	if(pos < 0) return;
	m_iStackDepth = 1;
	m_iFunpos[1] = atoi(funlist.Mid(pos+funname.GetLength()+1,funlist.Find(",",pos)-(pos+funname.GetLength()+1)));
	State = BETEST;
}
