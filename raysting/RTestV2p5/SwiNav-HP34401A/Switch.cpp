...
bool CSwitch::DoSwi(char *sParam)
{
	...
	m_Serial1.SendData('s',data,count+1);
	
		for(int i=0;i<3000;i++)
	{
		Sleep(1);
		if(m_Serial1.ReadDataWaiting() > 0)
		{
			Sleep(1);
			break;
		}
	...
	cpos = m_Serial1.ReadData(data,MAXSWISTORE);
...
bool CSwitch::Open(int portnum)
{
	...
	while(nPort++ < 8){
		ErrFlag = NO_ERR;
		if(nPort > 0)
			m_Serial1.SetPort(nPort);
		if(!m_Serial1.Open(portnum,9600,0,0,8))
			continue;
		...
		while(m_Serial1.ReadDataWaiting() > 0)
			m_Serial1.ReadData(&data,1);
		if(!DoAction(QMARK))
		{
			m_Serial1.Close();
			bOpen = false;
...
int cpos,i;
	...
	cpos = m_Serial1.ReadData(data,4);
