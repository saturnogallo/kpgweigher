...
{ 
    ...
    /* Initialize USART0, Placed after boot_vector.baud_rate initialization. */
    UART_Init(); 
   ...
   /*******************************************************************************/   
    if((boot_read_program_memory(LAST_APP_PAGE) != 0xAA55) || FORCE_FW_UPGRADE )
        boot_vector.boot_cmd = UPGRADE_FIRMWARE; 
    
    while(1){
       /******************************************************************************************************/    
       /* if command received is to update firmware, waiting for data from master board.*/
       /******************************************************************************************************/    
       if((boot_vector.boot_cmd == UPGRADE_FIRMWARE) ){
          if(tick++ > 25000)
          ...
          if(boot_comm.upgrade_cmd == FW_UPGRADE_CMPT){
              ...
              boot_comm.page_buffer[0] = 0xAA;            /* program 0xAA55 (firmware upgrade successfully */   
          ...
          if(boot_comm.upgrade_cmd == RUN_APP){
             boot_vector.boot_cmd = NORMAL_BOOT;              /* clear command on completion*/
             execute_app_code();           
          }                                    
       ...
       /******************************************************************************************************/            
       if(boot_vector.boot_cmd == NORMAL_BOOT)            
           execute_app_code();
