...
double RValueToTValue(double r, uchar prbid)
{
	...
	ac = prbdata.param1[prbid-1];
	bc = prbdata.param2[prbid-1];
	cc = prbdata.param3[prbid-1];
...
double GetWr(double t)
{
	...
	double pert;
	int i;
	const double Ci[] = {2.78157254, 1.64650916 ,-0.13714390, -0.00649767 ,-0.00234444, 0.00511868 ,0.00187982, -0.00204472, -0.00046122 ,0.00045724};

	const double Ai[] = {-2.13534729 ,3.18324720 ,-1.80143597, 0.71727204 ,0.50344027, -0.61899395, -0.05332322, 0.28021362, 0.10715224, -0.29302865, 0.04459872 ,0.11868632 ,-0.05248134};			 
	if(t < 0)
	{
		result = Ai[0];
...
double GetT(double w)
{
	...
	double result;
	int i;
	const double Di[] = {439.932854 ,472.418020, 37.684494 ,7.472018 ,2.920828 ,0.005184, -0.963864, -0.188732, 0.191203, 0.049025};
	const double Bi[] = {0.183324722, 0.240975303, 0.209108771, 0.190439972, 0.142648498, 0.077993465, 0.012475611, -0.032267127, -0.075291522, -0.056470670, 0.076201285, 0.123893204, -0.029201193, -0.091173542, 0.001317696, 0.026025526};
	if(w >= 0)
	{
		perw = (w-2.64)/1.64;
		w = perw;
