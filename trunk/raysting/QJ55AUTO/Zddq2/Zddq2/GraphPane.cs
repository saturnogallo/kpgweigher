...
namespace Zddq2
{
    public class PointPair : IComparable
    {
        ...
        public PointPair(int id, double val, bool ex, DateTime st)
        {
            index = id;
            value = val;
            stamp = st;
    ...
    public partial class GraphPane : UserControl
    {
        ...
        public void DrawLines(Graphics gHdc)
        {
            ...
            if (valid_points >= 1)
            {
                int first = first_point_index;
                previous = AxisTransfrom(PointList[first]);
                if (previous.Y < 2)
                    previous.Y = 4;
                ...
                for (int i = first+1; i < PointList.Count; i++)
                {
                    if (PointList[i].excluded)
                        continue;
        ...
        {
            double tempX = ((this.ClientRectangle.Width - 2 * this.origin.X) / this.XAxisMax) * trueValue.index + origin.X;
            double val = (trueValue.value-average)*1e6/average;
